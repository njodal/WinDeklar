...
def show_simple_window(title='Sine and Cosine graph', number_of_points=100, size=(5, 5),
    ...
    for [function_name, color] in functions:
        graph_one_function(function_name, window, number_of_points, color)
    window.show()


def graph_one_function(function_name, window, number_of_points, color):
    points, msg = ga.graph_points_for_many_functions(function_name, number_of_points)
    if points is None:
        print('%s not implemented' % function_name)
        return
    ga.graph_points(window.ax, points, color=color)
